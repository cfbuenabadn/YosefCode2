...
loadCommonPreprocOutput = function(collect_dir ,config_file, qc_fields_file, gene_fields_file)
{
  ...
  {
    stop(sprintf("There are %d samples that appear in the config file but do not appear either in the collected data!\nFor example: %s", sum(samplesInConfigButNotInCollectedData), config_table$output_name[which(samplesInConfigButNotInCollectedData)[1]]))
  }
