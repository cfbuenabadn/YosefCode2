...
# Directories containing your RNA Seq results from the preproc pipeline, collected.
# Example: c("/data/yosef/users/mbcole/Fluidigm/collect2/")
collect_dirs = c("/data/yosef2/HIV/dat/CMV_HIV_150wk/co_A3001-B04/",
                 "/data/yosef2/HIV/dat/CMV_HIV_150wk/co_A3001-B17/",
                 "/data/yosef2/HIV/dat/CMV_HIV_150wk_b2/co_rerun/")

# Config files for your project (.xls or .xlsx).
...
config_files = c("/data/yosef2/HIV/dat/CMV_HIV_150wk/A3001_B04_config.xlsx",
                 "/data/yosef2/HIV/dat/CMV_HIV_150wk/A3001_B17_config.xlsx",
                 "/data/yosef2/HIV/dat/CMV_HIV_150wk_b2/B3701_rerun_config.xlsx")

# Output directory
# Example: /data/yosef/users/mbcole/Fluidigm/out_test
out_dir = "/data/yosef2/HIV/dat/CMV_HIV_150wk/summary_out_rerun"
...
# Loads all of the count results into R
source(paste0(lib_dir,"/loadProcessedRNASeq_NG.R"))

eSet = NULL
for(i in 1:length(collect_dirs)){  
  collect_dir = collect_dirs[i]
  ...
  eSet_part = dataSet$rsem_eSet
  exprs(eSet_part) = assayData(eSet_part)$expectedCounts_table
  if(!is.null(eSet)){
    eSet = Biobase::combine(eSet,eSet_part)
  }else{
    eSet = eSet_part
  }
}
...
prop.failed = (dim(sc.eSet)[2] - 10)/dim(sc.eSet)[2] # In MORE than this fraction of cells...
# Will be removed.
