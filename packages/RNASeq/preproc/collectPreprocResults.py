        ...
        for cell_ind in xrange(NUM_CELLS):
            ...
            if(os.path.exists(cuffOutputGenes)):
                with open(cuffOutputGenes) as fin:
                    next(fin); #skip the first (header) line
                    rows = ( line.strip().split('\t') for line in fin if not(line.startswith('\t'))) #the addition "line does not start with \t" is required for the zebrafish cuff dictionary because in it you have genes with no gene id (in the cuff dictionary I removed them, but they will still be present in the cufflinks fpkm files)
