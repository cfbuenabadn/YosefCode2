...
includeFilter = [z.strip() for z in open("/home/eecs/allonwag/temp/zzz.txt").readlines()]
...
for sample1 in sampleList:
	...
	if( not(sampleName in includeFilter) ):
		print "sample "+ sampleName + " not in list of desired samples - skipping it..."
		continue
	else:
		print "sample "+ sampleName + " is in the list of desired samples - processing it..."
	...
	cmd = Template("python $SCRIPT_TO_RUN $IS_PAIRED_END \
					...
					$KALLISTO_BOOTSTRAP_SAMPLES $MEAN_FRAGMENT_LENGTH $STD_FRAGMENT_LENGTH $SINGLE_OR_BULK_PRIOR $SAMPLE1 $SAMPLE2").substitute(
		...
		SINGLE_OR_BULK_PRIOR="--single_cell_prior" if args.single_cell_prior else "--bulk_prior",
		SAMPLE1=sample1,
