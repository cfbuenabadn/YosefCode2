...
DO_TRIMMOMATIC = True;
if(DO_TRIMMOMATIC and not(args.skip_trimmomatic)):
	print("**********************************************************");
	print("**********************************************************");
...
if(RUN_RSEM_PIPELINE and not(args.skip_rsem)):	
	...
	if(args.paired_end):
		bowtieCommand = Template("bowtie2 -q --phred33 --sensitive --dpad 0 \
		--gbar 99999999 --mp 1,1 --np 1 --score-min L,0,-0.1 -I 1 -X 1000 \
		--no-mixed --no-discordant -p $NUM_THREADS -k 200 \
		-x /data/yosef/index_files/mm10_4brain/index/rsem_index/GRCm38.p3_4brain_rsem \
		-1 $OUTPUT_FOLDER//trimmomatic_output/1.Ptrim.fastq \
		-2 $OUTPUT_FOLDER/trimmomatic_output/2.Ptrim.fastq \
		 | samtools view -S -b -o $OUTPUT_FOLDER/rsem_output/rsem_output.bam -").substitute(OUTPUT_FOLDER=args.output_folder, NUM_THREADS=args.num_threads);
	else:
		bowtieCommand = Template("bowtie2 -q --phred33 --sensitive --dpad 0 \
		--gbar 99999999 --mp 1,1 --np 1 --score-min L,0,-0.1 -I 1 -X 1000 \
		--no-mixed --no-discordant -p $NUM_THREADS -k 200 \
		-x /data/yosef/index_files/mm10_4brain/index/rsem_index/GRCm38.p3_4brain_rsem \
		-U $OUTPUT_FOLDER/trimmomatic_output/1.Utrim.fastq \
		 | samtools view -S -b -o $OUTPUT_FOLDER/rsem_output/rsem_output.bam -").substitute(OUTPUT_FOLDER=args.output_folder, NUM_THREADS=args.num_threads);
	
	
	print(bowtieCommand)
	sys.stdout.flush();
	returnCode = subprocess.call(bowtieCommand, shell=True);
	if(returnCode != 0):
		raise Exception("bowtie2 failed");


	print("Sort SAM");
	sortSamCommand = Template("picard SortSam TMP_DIR=$OUTPUT_FOLDER/temp I=$OUTPUT_FOLDER/rsem_output/rsem_output.bam O=$OUTPUT_FOLDER/rsem_output/rsem_output_sorted.bam SO=coordinate").substitute(OUTPUT_FOLDER=args.output_folder);
	print(sortSamCommand)
	sys.stdout.flush();
	returnCode = subprocess.call(sortSamCommand, shell=True);
	if(returnCode != 0):
	...
	removeDupsCommand = Template("picard MarkDuplicates REMOVE_DUPLICATES=true \
	INPUT=$OUTPUT_FOLDER/rsem_output/rsem_output_sorted.bam \
	OUTPUT=$OUTPUT_FOLDER/rsem_output/rsem_output_duplicatesRemoved.bam \
	METRICS_FILE=$OUTPUT_FOLDER/rsem_output/dupInfo.txt").substitute(OUTPUT_FOLDER=args.output_folder);
	print(removeDupsCommand)
	sys.stdout.flush();
	returnCode = subprocess.call(removeDupsCommand, shell=True);
	if(returnCode != 0):
		raise Exception("sortSam failed");
		
		
	print "done duplication - aborting..."
	exit();
		
	print("Sort SAM");
	sortSamCommand = Template("picard SortSam TMP_DIR=$OUTPUT_FOLDER/temp I=$OUTPUT_FOLDER/rsem_output/rsem_output_duplicatesRemoved.bam O=$OUTPUT_FOLDER/rsem_output/rsem_output_duplicatedRemoved_sorted.bam SO=coordinate").substitute(OUTPUT_FOLDER=args.output_folder);
	print(sortSamCommand)
	sys.stdout.flush();
	returnCode = subprocess.call(sortSamCommand, shell=True);
	if(returnCode != 0):
	...
	if(args.paired_end):
		...
		rsemComand = Template("/opt/pkg/rsem-1.2.19/bin/rsem-calculate-expression --num-threads $NUM_THREADS --bam --estimate-rspd --output-genome-bam --sampling-for-bam --paired-end $OUTPUT_FOLDER/rsem_output/rsem_output_duplicatesRemoved_sorted.bam $RSEM_INDEX $OUTPUT_FOLDER/rsem_output/rsem_output").substitute(OUTPUT_FOLDER=args.output_folder, RSEM_INDEX=RSEM_INDEX, SAMPLE_FILE1=args.sampleFile1, SAMPLE_FILE2=args.sampleFile2, NUM_THREADS=args.num_threads);
	else:
