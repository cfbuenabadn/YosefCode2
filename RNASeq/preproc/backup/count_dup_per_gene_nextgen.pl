...
while(<FD_sam>){
	...
	if(0 && $only_uniquely_alligned){ #Disabled since the flag is mimssing (for some reason) from Bowtie output
        ...
        if(!($chrom=~/\*/ || $bitmap&0x200 || $bitmap&0x4 || $bitmap&0x8 || ($only_uniquely_alligned && ($bitmap&0x100)))){
			...
			my @genome_dat_5p=();my %genes_5p;my @genome_dat_3p=();my %genes_3p;
			read_genes(\@genome_dat_5p,\%genes_5p,$rsemGTFFile,1,$prv_chrom);
			read_genes(\@genome_dat_3p,\@genome_dat_3p,$rsemGTFFile,0,$prv_chrom);
			
			
			print STDERR  "Read ".scalar(keys %genes_5p)." 5\" loci\n";

			foreach my $k(keys %uread_no_strand){
				my($chr,$pos)=split(/\#/,$k);
				my @hits;get_overlapping_genes(\@hits,\@genome_dat_5p,\%genes_5p,\@genome_dat_3p,\@genome_dat_3p)
